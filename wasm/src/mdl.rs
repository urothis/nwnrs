...
use crate::{
    bindings::{error_message, js_error_message},
...
fn mdl_to_dto(bytes: &[u8]) -> Result<MdlDto, String> {
    let parsed = mdl::parse_model_bytes(bytes)
        .map_err(|error| error_message("failed to read MDL", error))?;
    match parsed {
        ...
        mdl::ParsedModel::Compiled(model) => Ok(MdlDto {
            ...
            text:     mdl::lower_binary_model_to_ascii(&model)
                .map_err(|error| error_message("failed to lower compiled MDL", error))?
                .to_text(),
            lossless: None,
        }),
    }
...
pub(crate) fn read_mdl_dto(bytes: &[u8]) -> Result<MdlDto, String> {
    ...
    match value.encoding {
        MdlEncodingDto::Ascii => {
            let model = mdl::parse_ascii_model(&value.text)
                .map_err(|error| error_message("failed to parse ASCII MDL", error))?;
            let mut out = Cursor::new(Vec::new());
            mdl::write_ascii_model(&mut out, &model)
                .map_err(|error| error_message("failed to write ASCII MDL", error))?;
            Ok(out.into_inner())
        }
        MdlEncodingDto::Compiled => Err(error_message(
...
wasm_read_binding! {
    fn read_mdl_from_bytes(bytes: &[u8]) -> MdlDto {
        read_mdl_dto(bytes).map_err(|error| js_error_message(&error))
    }
...
wasm_write_binding! {
    ...
    {
        write_mdl_dto(&value).map_err(|error| js_error_message(&error))
    }
}
