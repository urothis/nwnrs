...
pub struct ErfPackMetadata {
    ...
    pub file_md5s:             BTreeMap<String, String>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct KeyPackMetadata {
    pub source_key:     PathBuf,
    pub source_key_md5: String,
    pub bifs:           Vec<String>,
    pub bif_md5s:       BTreeMap<String, String>,
    pub file_md5s:      BTreeMap<String, String>,
}
...
pub fn write_erf_pack_metadata(
    ...
    let metadata_path = destination.join(PROJECT_LOCK_FILENAME);
...
pub fn write_new_key_pack_metadata(destination: &Path, force: bool) -> Result<(), String> {
    ...
    let value = KeyPackMetadataFile {
        ...
        source_key_md5: String::new(),
        version:        KeyPackMetadataVersion::V1,
        build_year:     0,
        build_day:      0,
        oid:            None,
        bifs:           Vec::new(),
        bif_md5s:       BTreeMap::new(),
        file_md5s:      snapshot_packable_files(destination)?,
    };
    fs::write(
        &metadata_path,
...
fn read_metadata_text(input: &Path) -> Result<Option<(PathBuf, String)>, String> {
    let metadata_path = if input.is_dir() {
        input.join(PROJECT_LOCK_FILENAME)
    } else if input.file_name().and_then(|value| value.to_str()) == Some(PROJECT_LOCK_FILENAME) {
        input.to_path_buf()
    } else {
        return Ok(None);
    };
...
fn parse_entry_order(
    ...
    entries
        .into_iter()
        .map(|entry| {
            resman::ResolvedResRef::from_filename(&entry)
                .map(Into::into)
                .map_err(|error| {
                    format!(
...
fn parse_entry_algorithms(
    ...
    entries
        .into_iter()
        .map(|(entry, algorithm)| {
            let rr = resman::ResolvedResRef::from_filename(&entry)
                .map(Into::into)
                .map_err(|error| {
                    format!(
...
mod tests {
    ...
    fn snapshot_excludes_control_files() {
        ...
        fs::write(root.join(PROJECT_LOCK_FILENAME), "{}").expect("write lock");
