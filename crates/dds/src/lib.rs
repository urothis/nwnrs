#![forbid(unsafe_code)]
//! Typed parser, decoder, writer, and RGBA encoder for Neverwinter Nights DDS
//! texture payloads.
//!
//! NWN stores DDS data as a compact 20-byte header followed by packed DXT mip
//! levels. This crate parses that NWN-specific payload format only.
...
mod tests {
    ...
    fn fixture_decodes_top_level_rgba8() {
        ...
        let rgba = dds.decode_rgba8().unwrap_or_else(|error| {
            panic!("decode fixture rgba8: {error}");
        });
