...
- [crate docs](https://docs.rs/nwnrs-tga/latest/nwnrs_tga/)
- [README](https://github.com/urothis/nwnrs/blob/main/crates/formats/tga/README.md)
- [source](https://github.com/urothis/nwnrs/blob/main/crates/formats/tga/src/lib.rs)

`nwnrs-tga` models NWN `TGA` textures as typed payloads rather than as
immediately-decoded images.
...
- Origin bits matter. Top-to-bottom and left-to-right orientation should not be
  normalized away silently.
- RLE-vs-non-RLE storage mode remains explicit through `TgaImageType`.
- Indexed-color and true-color modes are different physical layouts, even if a
  renderer eventually normalizes both to RGBA8.
- Raw trailing bytes and footer metadata are part of the typed texture because
  they are part of the authored file.
